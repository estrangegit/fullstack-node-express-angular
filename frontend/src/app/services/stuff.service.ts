...
export class StuffService {
  ...
  getStuff() {
    this.http.get('/api/stuff').subscribe(
      (stuff: Thing[]) => {
        if (stuff) {
          this.stuff = stuff;
          this.emitStuff();
  ...
  getThingById(id: string) {
    return new Promise((resolve, reject) => {
      this.http.get('/api/stuff/' + id).subscribe(
        (response) => {
          resolve(response);
        },
        (error) => {
  ...
  createNewThingWithFile(thing: Thing, image: File) {
    return new Promise((resolve, reject) => {
      ...
      thingData.append('image', image, image.name);
      this.http.post('/api/stuff', thingData).subscribe(
        (response) => {
          resolve(response);
        },
        (error) => {
